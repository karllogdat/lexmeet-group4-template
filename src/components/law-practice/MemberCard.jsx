...
export default function MemberCard({ name, note, active = false, photoSrc, fluid = false }) {
  return (
    <div
      className={`${fluid ? "w-full" : "w-44 sm:w-52"} rounded-xl overflow-hidden bg-white/5 transition-transform duration-300 ${
        active ? "scale-110 z-10 ring-2 ring-white" : "scale-100"
      }`}
    >
