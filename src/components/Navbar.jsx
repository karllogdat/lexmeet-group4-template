  ...
  const handleNavClick = (e, item) => {
    ...
    if (item.label === "Our Services") {
      ...
      if (location.pathname !== "/") {
        navigate("/#our-services", { state: { scrollToServices: true } });
      } else {
          ...
          <div className="hidden items-center justify-center gap-2 xl:flex xl:gap-4 2xl:gap-6">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span
                    className={`whitespace-nowrap text-xs xl:text-sm 2xl:text-base transition-colors ${
                      active
                        ? "font-bold text-sky-400 border-b-2 border-sky-400 pb-1"
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={`py-2.5 ${
                      active ? "font-bold text-sky-400" : "text-white"
                    }`}
                  >
                    {item.label}
                  </div>
